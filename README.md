...
- ⚡ **一键注入 CAD**：可从已检测到的多个 AutoCAD 实例及图纸中选择目标，自动生成 `GB2312` 编码的 `.lsp` 文件，并通过 COM 接口加载到所选图纸中。
...
3. **注入 CAD**：
   ...
   - 先在“选择CAD实例”下拉框中选择目标实例，再在上方图纸下拉框中选择该实例内的目标图纸；后打开 CAD 或图纸后，可点击刷新按钮重新检测。
...
如果需要将源码打包为独立程序发给他人使用，可使用项目中的 PyInstaller 配置：

```bash
python -m PyInstaller --noconfirm "CAD字段变量管理器.spec"
```
> 打包完成后，程序位于 `dist/CAD字段变量管理器.exe`。替换旧版程序时保留原目录下的 `data/projects_data.json`，以免丢失已有项目。
